...
class _NumberPickerState extends State<NumberPicker> {
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    final child = widget.isCustom ? Padding(
      padding:  EdgeInsets.only(left:(value==0)?MediaQuery.of(context).size.width / 2.3: MediaQuery.of(context).size.width / 2.5),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Text(getDisplayedValue(value),style: TextStyle(fontSize: 24,color: Colors.blue)),
          SizedBox(width: 8,),
          if(value % 1000 == 0 && value != 0)
          Text("${value ~/ 1000} sec",style: TextStyle(fontSize: 20,color: Colors.pink)),
        ],
      ),
    ) : ( isExtra
        ? SizedBox.shrink()
